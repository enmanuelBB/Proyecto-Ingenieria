...
public class DataSeeder {
    ...
    public void crearEncuestaPredeterminada(EncuestaRepository encuestaRepository) {
        ...
        preguntas.add(crearPregunta(encuesta, "Nombre completo", "TEXTO_LIBRE", true));
        preguntas.add(crearPregunta(encuesta, "Teléfono", "TEXTO_LIBRE", true));
        preguntas.add(crearPregunta(encuesta, "Correo electrónico", "TEXTO_LIBRE", true));
        preguntas.add(crearPregunta(encuesta, "Código del participante", "TEXTO_LIBRE", true));
        preguntas.add(
                crearPreguntaSeleccion(encuesta, "Grupo", true, Arrays.asList("Caso (cáncer gástrico)", "Control")));
        preguntas.add(crearPregunta(encuesta, "Fecha de inclusión", "TEXTO_LIBRE", true)); // Podría ser DATE pero
                                                                                           // TEXTO_LIBRE es más seguro
                                                                                           // por ahora

        // 2. Datos sociodemográficos
        preguntas.add(crearPregunta(encuesta, "Edad", "NUMERO", true));
        preguntas.add(crearPreguntaSeleccion(encuesta, "Sexo", true, Arrays.asList("Hombre", "Mujer")));
        preguntas.add(crearPregunta(encuesta, "Nacionalidad", "TEXTO_LIBRE", true));
        preguntas.add(crearPregunta(encuesta, "Dirección", "TEXTO_LIBRE", true));
        preguntas.add(crearPregunta(encuesta, "Comuna", "TEXTO_LIBRE", true));
        preguntas.add(crearPregunta(encuesta, "Ciudad", "TEXTO_LIBRE", true));
        preguntas.add(crearPreguntaSeleccion(encuesta, "Zona", true, Arrays.asList("Urbana", "Rural")));
        preguntas.add(crearPreguntaSeleccion(encuesta, "¿Vive usted en esta zona desde hace más de 5 años?", true,
                Arrays.asList("Sí", "No")));
        ...
        preguntas.add(crearPreguntaSeleccion(encuesta,
                "Diagnóstico histológico de adenocarcinoma gástrico (solo casos)", false, Arrays.asList("Sí", "No")));
        ...
        preguntas.add(crearPregunta(encuesta,
                "Otras enfermedades relevantes (ej. gastritis crónica, úlcera péptica, anemia)", "TEXTO_LIBRE", false));
        preguntas.add(crearPreguntaSeleccion(encuesta, "Uso crónico de medicamentos gastrolesivos (AINES u otros)",
        ...
        preguntas.add(crearPregunta(encuesta, "Índice de masa corporal (IMC)", "NUMERO", true));
        ...
        preguntas.add(crearPreguntaSeleccion(encuesta, "Frecuencia consumo alcohol", false,
                Arrays.asList("Ocasional (<1 vez/sem)", "Regular (1–3 veces/sem)", "Frecuente (≥4 veces/sem)")));
        ...
        preguntas.add(
                crearPreguntaSeleccion(encuesta, "Consumo de alimentos muy salados", true, Arrays.asList("Sí", "No")));
        preguntas.add(crearPreguntaSeleccion(encuesta, "Consumo de porciones de frutas y verduras frescas", true,
                Arrays.asList("≥5 porciones/día", "3–4 porciones/día", "≤2 porciones/día")));
        preguntas.add(crearPreguntaSeleccion(encuesta, "Consumo frecuente de frituras (≥3 veces por semana)", true,
                Arrays.asList("Sí", "No")));
        ...
        preguntas.add(crearPreguntaSeleccion(encuesta, "¿Recibió tratamiento para erradicación de H. pylori?", false,
                Arrays.asList("Sí", "No", "No recuerda")));
        preguntas.add(crearPregunta(encuesta, "Si sí, año y esquema", "TEXTO_LIBRE", false));
        preguntas.add(
                crearPreguntaSeleccion(encuesta, "Tipo de examen realizado", false, Arrays.asList("Test de aliento",
                        "Antígeno en deposiciones", "Serología", "Test rápido de ureasa", "Histología", "Otro")));
        ...
        preguntas.add(crearPreguntaSeleccion(encuesta, "Uso de antibióticos o IBP en las 4 semanas previas", false,
                Arrays.asList("Sí", "No", "No recuerda")));
        preguntas.add(crearPreguntaSeleccion(encuesta, "¿Ha repetido el examen posteriormente?", false,
                Arrays.asList("Sí", "No")));
        preguntas.add(crearPregunta(encuesta, "Si repitió, fecha y resultado más reciente", "TEXTO_LIBRE", false));
